...
export default function Profile() {
  const { user, profileImage, setProfileImage, login, hasPaid } = useAuth();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // ✅ Set profile image from backend photo_url if available
        if (profile.photo_url) {
          setProfileImage(profile.photo_url);
          localStorage.setItem("profileImage", profile.photo_url);
        }
  ...
  }, [hasFetched, login, setProfileImage]);

  // ✅ Upload profile image to Firebase and send URL to backend
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      // 2️⃣ Update backend
      const res = await updateUserProfile({ photo_url: photoURL });

      // 3️⃣ Sync new user data globally
      login(res.data.user, localStorage.getItem("token"));

      // 4️⃣ Save and display new image
  ...
  const admissionStatus = hasPaid ? "Student" : "Pending";
  const applicationStatus = hasPaid ? "Approved" : "Pending";
    ...
    <div className="p-6 max-w-2xl mx-auto bg-[#FBFBFB] rounded-md shadow-sm relative">
      ...
      <div className="flex flex-col items-start">
        ...
        <div className="mt-6 space-y-3 text-[#4D4D4D] w-full text-sm">
          ...
          <p>
            ...
            <span className={hasPaid ? "text-green-600" : "text-yellow-600"}>
              {applicationStatus}
            </span>
          </p>
          <p>
            <strong>Admission Status:</strong>{" "}
            <span className={hasPaid ? "text-blue-600" : "text-yellow-600"}>
              {admissionStatus}
            </span>
          </p>
        </div>
      ...
      {(uploading || saving) && (
        <div className="absolute inset-0 bg-white/60 flex items-center justify-center rounded-md">
          <p className="text-[#001299] font-medium">
...
// export default function Profile() {
//   const { user, profileImage, setProfileImage, login, hasPaid } = useAuth(); // ✅ now using hasPaid directly
//   const studentId = formatDateJoinedWithId(user);
...
//   const initials = getInitials(user);
//   const admissionStatus = hasPaid ? "Student" : "Pending"; // ✅ now uses context
//   const applicationStatus = hasPaid ? "Approved" : "Pending"; // ✅ now uses context
